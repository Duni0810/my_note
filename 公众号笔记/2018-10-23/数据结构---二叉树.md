...
这两种方法我觉得第二种方法会好很多，因为利用结点空指针线索化的方法会破坏树的结构，使用完之后不能够再恢复。虽然这两个问题可以在树结点中加入一个线索化指针而得以解决，然而线索化指针的加入又会浪费内存空间，不够灵活。
使用链表线索化方法不会破化树的结构，不需要时线索化时销毁链表
即可；链表线索化方法可以很容易的以任何一种遍历顺序对二叉树进行
线索化。

### 霍夫曼树

 这东西怎么来的呢？我第一次接触是大学本科的时候，上图像处理的老师给我们讲解压缩与解压缩的时候接触到，那他到底是什么东西呢？举一个例子：
 假如我们对“ABCDEF”进行编码，如下：
