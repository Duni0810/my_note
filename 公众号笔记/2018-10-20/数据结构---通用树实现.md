...
利用上面的关系就可以将数的关系理清楚，但是怎么存储还是没清晰。我们可以创建一个通链，这个通链就是将树中的各个节点串一起，然后每个树节点自己管理自己的自己的孩子节点，孩子节点也是通过链表串成。

我们利用链表组织树中的各个结点，链表中的前后关系不代表结点间的逻辑关系，结点的逻辑关系由child数据域描述，child数据域保存其他结点的存储地址。
