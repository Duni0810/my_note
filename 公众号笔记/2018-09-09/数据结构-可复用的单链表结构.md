...
函数中做了好多的强制类型的转化，这个是因为结构体的第一个元素是指向下一个地址的指针，首元素的地址和该结构体的地址是一样的，你这么理解就好理解多了。
函数首先得有一个参数合法性的判断，保证正确性。在上面的代码中，有一个判断不能丢失`(slist->hander.next != NULL)`,防止越界。我在 `list.c` 文件中还有一个参数就是结构体的长度
