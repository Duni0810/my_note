...
看吧，运行一切正常。如果我们一开始使用的是这样的接口，那添加新的驱动就太容易了，只需要向接口层提供一套功能函数就可以了，其他代码都不用动的 ，对于用户来说，还一个驱动只需要还一个初始化函数而已，其他不用改动，是不是将代价降到了最低。不仅仅如此，我们如果思维在开阔点，我们使用这个方式是不是可以实现驱动的跨平台呢？
